...
@HEADS.register_module()
class ProbConvFCIoUBBoxHead(BBoxHead):
    ...
    def __init__(self,
                 ...
                 norm_cfg=None,
                 init_cfg=None,
                 loss_iou=dict(type='CrossEntropyLoss', use_sigmoid=True, loss_weight=0.5),
                 *args,
                 **kwargs):
        super(ProbConvFCIoUBBoxHead, self).__init__(
            *args, init_cfg=init_cfg, **kwargs)
        assert (num_shared_convs + num_shared_fcs + num_cls_convs +
                num_cls_fcs + num_reg_convs + num_reg_fcs > 0)
        if num_cls_convs > 0 or num_reg_convs > 0:
        ...
        self.iou_convs, self.iou_fcs, self.reg_last_dim = \
            self._add_conv_fc_branch(
                self.num_reg_convs, self.num_reg_fcs, self.shared_out_channels)
    ...
    def forward(self, x):
        ...
        cls_score = self.fc_cls(x_cls) if self.with_cls else None
        bbox_pred = self.fc_reg(x_reg) if self.with_reg else None
        iou_pred = self.fc_iou(x_iou) if self.with_reg else None
            ...
            if pos_inds.any():
                ...
                if self.focal_reg:
                    ...
                    losses['loss_bbox'] = self.loss_bbox(
                        ...
                        reduction_override=reduction_override)
#                 print(iou_pred.shape)
#                 print(iou_target.shape)
                losses['loss_ious'] = self.loss_iou(
                    iou_pred[pos_inds.type(torch.bool)].reshape(-1),
                    iou_target,
                    avg_factor=bbox_targets.size(0),
                    reduction_override=reduction_override)
            else:
                losses['loss_bbox'] = bbox_pred[pos_inds].sum()
                losses['loss_ious'] = bbox_pred[pos_inds].sum()
        if cls_score is not None:
            avg_factor = max(torch.sum(label_weights > 0).float().item(), 1.)
            if cls_score.numel() > 0:
                if isinstance(self.loss_cls, QualityFocalLoss):
