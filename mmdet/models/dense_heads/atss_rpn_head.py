...
from ..losses import GHMR,VarifocalLoss
...
class Mish(nn.Module):
    ...
    def forward(self, x):
        ...
        return x
def norm_loss(loss, weights, avg_factor):
        ...
        if len(pos_inds) > 0:
            ...
            if self.reg_decoded_bbox:
                ...
                if self.with_aug_loss:
                    ...
                    bbox_weights_aug = (bbox_weights_aug * (iou_target**self.gamma)[:,None]).clamp(min=EPS)
                    # loss_box_aug = self.aug_loss(pos_bbox_pred, pos_encode_bbox_targets, bbox_weights_aug.clamp(min=EPS),avg_factor=1.0)
                    loss_box_aug = self.aug_loss(pos_bbox_pred, pos_encode_bbox_targets, bbox_weights_aug,reduction_override='none')
                    loss_box_aug = norm_loss(loss_box_aug, bbox_weights_aug, 1.0)

                bbox_weights = (iou_target**self.gamma).clamp(min=EPS)
                # regression loss
                ...
                loss_bbox = norm_loss(loss_bbox, bbox_weights, 1.0)
...
#                 avg_factor = (iou_target).sum()
                avg_factor = iou_target.new_tensor(pos_bbox_pred.shape[0])
