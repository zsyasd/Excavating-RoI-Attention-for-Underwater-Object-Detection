...
class ProbRoIHead(StandardRoIHead):
    def __init__(self, alpha=0, gamma=0.1, boost=False, prob=True, ams=False, quality=False, iou_gamma=0, reg_norm='mean', **kwargs):
...
#         scores = torch.cat([boxes[:,-2] for boxes in proposals],dim=0)
#         iou_preds = torch.cat([boxes[:,-1] for boxes in proposals],dim=0)
#         print(scores.shape)
#         print(iou_preds.shape)
#         prior = (iou_preds*scores)**0.5
...
@HEADS.register_module()
class IoUProbRoIHead(ProbRoIHead):
    def __init__(self, **kwargs):
        super(IoUProbRoIHead, self).__init__(**kwargs)
    ...
    def _bbox_forward_train_boost(self, x, sampling_results, gt_bboxes, gt_labels,
        ...
        loss_bbox['loss_cls'] = (loss_bbox['loss_cls']*cls_weights.detach()).mean()
        loss_bbox['loss_bbox'] = loss_bbox['loss_bbox'].mean()
        loss_bbox['loss_ious'] = loss_bbox['loss_ious'].mean()
        bbox_results.update(loss_bbox=loss_bbox)
        return bbox_results
    ...
    def _bbox_forward(self, x, rois):
        ...
        bbox_results = dict(
            cls_score=cls_score, bbox_pred=bbox_pred, iou_pred=iou_pred, bbox_feats=bbox_feats)
        return bbox_results
...
#         prior = (iou_preds*scores)**0.5
        ...
        if self.prob:
            ...
            cls_score = bbox_results['cls_score']
        bbox_pred = bbox_results['bbox_pred']
        iou_pred = bbox_results['iou_pred'].sigmoid()
        num_proposals_per_img = tuple(len(p) for p in proposals)
        rois = rois.split(num_proposals_per_img, 0)
        cls_score = cls_score.split(num_proposals_per_img, 0)
